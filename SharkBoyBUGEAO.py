...
class TradingOperator(QObject):
    ...
    def predict_state(self, latest_row):
        """
        Predice el estado actual basado en el modelo VariationalGaussianHMM.
        """
        try:
            ...
            if hasattr(self.model, "predict"):
                hidden_states = self.model.predict(input_df)
                state = hidden_states[-1]  # Último estado predicho
                print(f"[DEBUG] 🧠 Estado calculado: {state}")
                return state
            else:
                print("[ERROR] ❌ El modelo no tiene 'predict'. No se puede calcular estado.")
                return None
        except Exception as e:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    print("[DEBUG] latest_row:", latest_row.to_dict())  # Ver datos antes de extraer features

                    if all(f in latest_row for f in self.features):  # Verifica que todas las features existen en latest_row
                        input_features = [list(latest_row[self.features])]
                    else:
                        missing_features = set(self.features) - set(latest_row.index)
                        print(f"[ERROR] ❌ Algunas features faltan en latest_row: {missing_features}")
                        time.sleep(interval)
                        continue
    ...
    def update_balance_and_positions(self):
        try:
            ...
            # DEPURACIÓN: Imprimir contenido real antes de la validación
            print("[DEBUG] Contenido de 'positions':", positions)

            # ✅ Validar que 'positions' es una lista
            if isinstance(positions, tuple):  
                positions = positions[1]  # Extraer la segunda lista de la tupla si es necesario
        ...
        try:
            ...
            if num_sell_positions >= max_sell_positions:
                ...
                log_entry = {
                    ...
                    "reason": "Max sell positions reached",  # Agregamos la razón aquí
