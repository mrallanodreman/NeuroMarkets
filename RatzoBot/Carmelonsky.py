...
class TradingOperator(QObject):
    ...
    def process_data(self, row, positions, balance, state):
        ...
        try:
            ...
            num_buy_positions = len(buy_positions)  # ✅ Número actual de posiciones BUY
            max_buy_positions = self.capital_ops.max_buy_positions  # ✅ Límite definido en `CapitalOP`


            # 📊 Imprimir cuántas posiciones `BUY` y `SELL` hay antes de decidir
            print(f"[INFO] 📊 Posiciones actuales: BUY={len(buy_positions)}, SELL={num_buy_positions} (Máx permitido: {max_buy_positions})")
            ...
            # 🛑 Si `BUY` está en el límite, registrar en el log y hold
            if num_buy_positions >= max_buy_positions:
                print(f"[INFO] 🚨 Límite de posiciones BUY alcanzado. No se abrirá una nueva posición.")
                log_entry = {
            ...
            # 🔥 Ejecutar acción solo si es `BUY`
    ...
    def print_log(self):
        ...
        if self.log_open_positions:
            ...
            for entry in self.log_open_positions:
                ...
                print(f"📊 Posiciones abiertas: BUY={num_buy}, SELL={num_sell} ")
